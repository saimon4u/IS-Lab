...
string generateOutput(){
    return decimalToHexa(A) + decimalToHexa(B) + decimalToHexa(C) + decimalToHexa(D) + decimalToHexa(E) + decimalToHexa(F) + decimalToHexa(G) + decimalToHexa(H);
}
...
string SHA(string input){
    ...
    writeInFile("Hash-Value.txt", output);
    return output;
}
...
unsigned char key[] = "saimonbhuiyan123" ;
int extendedLen = 16 ;
string Cipher_text ;
...
void substitutionWord( unsigned char* input ){
    for (int i = 0; i <= 3; i++) input[i] = sBox[input[i]] ;
}

void rotationOfWord( unsigned char* input ){
    unsigned char temp = input[0] ;
    for( int i = 0; i <= 2; i++ ) input[i] = input[i+1];
    input[3]= temp;
}

void gFunction(unsigned char *prevWord, int index){
...
void expansionKey( unsigned char *originalKey ){
    int index ;
    for ( index = 0; index < 1; index++ ) ExpandedKey[index] = originalKey[index] ; 

    while ( index < 176 ){
        unsigned char prevWord[4] ;
        for ( int i = 0; i < 4; i++ ) prevWord[i] = ExpandedKey[index + i - 4] ; 

        if( (index%16) == 0 ){
            gFunction(prevWord, index);
        }

        for ( int k = 0; k < 4; k++ ) ExpandedKey[index+k] = prevWord[k]^ExpandedKey[index-16+k] ;
        index += 4 ;
    }
...
void addRoundKey( unsigned char* state, int roundNum ){
    for ( int i = 0; i < 16; i++ ) state[i] ^= ExpandedKey[roundNum*16+i] ;
}


void aesEncryption( unsigned char* state ){
    addRoundKey( state, 0 ) ;
    for ( int i = 1; i <= roundNumber; i++ ){
        subBytes( state ) ;
        shiftRow( state ) ;
        if( i != roundNumber ) mixColumn( state ) ;
        addRoundKey( state, i ) ;
    }
}

void addRoundKeyForDecryption( unsigned char* state, int roundNum ){
    for ( int i = 0; i < 16; i++ ) state[i] ^= ExpandedKey[160-roundNum*16+i] ;
}
...
void aesDecryption( unsigned char* Cipher_text ){
    addRoundKeyForDecryption( Cipher_text, 0 ) ;
    for ( int i = 1; i <= roundNumber; i++ ){
        inverseShiftRows( Cipher_text ) ;
        inverseSubBites( Cipher_text ) ;

        addRoundKeyForDecryption( Cipher_text, i );

        if( i != roundNumber ) inverseMixColumn( Cipher_text ) ;
    }
}

string cbcEncryption( string input ){
    int txtLen = input.length() ;
    unsigned char temp[16] ;

    if( (txtLen%16) != 0 ) extendedLen = txtLen + ( 16 - (txtLen%16) ) ;
    else extendedLen = txtLen ;
    Cipher_text.resize(extendedLen);
    unsigned char paddedText[extendedLen+1];

	for( int i=0; i<extendedLen; i++ ){
  	    if( i < txtLen ) paddedText[i] = input[i] ;
  	    else paddedText[i] = paddingCharacter ;
     
    }
  
    paddedText[extendedLen] = '\0';
    expansionKey( key ) ;
    string IV = "1011100101001010";
    for( int i=0; i<extendedLen; i+=16){
		for( int j=0; j<16; j++ ) temp[j] = paddedText[i+j] ^ IV[j];
		aesEncryption( temp );
		for( int j=0; j<16; j++ ){
            paddedText[i+j] = temp[j];
            IV[j]=temp[j];
        }
	}
    ...
    for ( int i = 0; i < extendedLen; i++){
        Cipher_text[i] = paddedText[i] ;
    }
    writeInFile("CBC-Encrypted.txt", Cipher_text);
    return Cipher_text;
}

string cbcDecryption( string Cipher_text ){
    ...
    for ( int i = 0; i < extendedLen; i += 16){
        ...
        aesDecryption(temp);
		for( int j=0; j<16; j++ ){
            decryptedTxt[i+j] = temp[j] ^ IV[j] ;
            IV[j]=Cipher_text[i+j];
        }
    }
    string decryptedText;
    decryptedText.resize(extendedLen);
    for(int i=0; i<extendedLen; i++){
        decryptedText[i]=decryptedTxt[i];
        if( decryptedTxt[i+1]==paddingCharacter && decryptedTxt[i+2]==paddingCharacter ){
            decryptedText[i+1] = '\0';
            decryptedText.resize(i+1) ;
            break ;
        }
    }
    writeInFile("CBC-Decrypted.txt", decryptedText);
    return decryptedText ;
}


string counterEncryption( string input ){
    int txtLen = input.length() ;
    unsigned char temp[16] ;
    ...
    for( int i=0; i<extendedLen; i+=16, counter++ ){
        ...
        counterString = fixedstream.str();
		for( int j=0; j<16; j++ ) temp[j] = counterString[j];    
		aesEncryption( temp ) ;
		for( int j=0; j<16; j++ ) paddedText [i+j] = temp[j] ^paddedText [i+j];
	}
    for ( int i = 0; i < extendedLen; i++) Cipher_text[i] = paddedText[i] ;
    writeInFile("Counter-Encrypted.txt", Cipher_text);
    return Cipher_text;
}

string counterDecryption( string Cipher_text ){
    unsigned char decryptedTxt[extendedLen] ;
    unsigned long long int counter = 1;
    string counterString;
    for ( int i = 0; i < extendedLen; i += 16 ,counter++){
        ...
        for( int j=0; j<16; j++ ) temp[j] = counterString[j];
        aesEncryption(temp);
		for( int j=0; j<16; j++ ) decryptedTxt[i+j] = Cipher_text[i+j]^temp[j];
    }
    string decr;
    ...
    writeInFile("Counter-Decrypted.txt", decr);
    return decr ;
}